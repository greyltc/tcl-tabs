# From SDX Tclkit package, BSD license
...
proc ::stringfileinfo::writeStringInfo {file array} {
    ...
    set data [read $fh]
    if {[info exists val(ProductVersionBinary)]} {
        set verinfo [split $val(ProductVersionBinary) .]
        for {set i 0} {$i < 4} {incr i} {
            set v [lindex $verinfo $i]
            if {(![string is integer -strict $v]) || ($v < 0) || ($v > 65535)} {
                set v 0
            }
            set v$i $v
        }
        set s [string first [utf16le "VS_VERSION_INFO\0"] $data]
        if {$s > 0} {
            seek $fh [expr {$s + 42}] start
            puts -nonewline $fh \
                [binary format s4 [list $v1 $v0 $v3 $v2]]
        }
        unset val(ProductVersionBinary)
    }
    ...
    foreach x [array names val] {
        # ignore ProductVersionBinary as it is set as binary value
        if {$x == "ProductVersionBinary"} {
            continue
        }
